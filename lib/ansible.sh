...
devvm_inventory_line() {
	...
	printf ' devvm_upgrade_packages=%s' "$(devvm_ini_quote "$DEVVM_UPGRADE_PACKAGES")"
	printf ' code_dir=%s' "$(devvm_ini_quote "$CODE_DIR")"
	printf ' install_node=%s' "$(devvm_ini_quote "$INSTALL_NODE")"
	...
	printf ' ai_allow_http_model_urls=%s' "$(devvm_ini_quote "$AI_ALLOW_HTTP_MODEL_URLS")"
